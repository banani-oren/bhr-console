...
- Current pending prompts: `cleanup-d141e376-billing-event.md` — a separate open item (a real duplicate-index billing_events anomaly on one transaction, flagged during Repair 13 housekeeping) still awaiting Oren's review before any deletion; do not action without re-verifying intent. All other known prompts done & live (see Phase History for full history: `repair5`/`repair5-complete`, `repair5b`, `repair6`, `repair8`, `repair9`, `repair10`, `repair11`, `repair12`, `feature-impersonate-user`, `fix-impersonate-deploy`, `feature-attendance-v2`, `housekeeping-repair13`, `repair14-save-hang`, `feature-approval-notification`, `feature-attendance-admin`, `feature-billing-events`, `feature-billing-reports`, `feature-form-improvements`, `feature-mobile-redesign`, `run-all-phases`, `fix-save-rootcause-auth-deadlock`, `batch8-mobile-reports-giyus-billing`, `feature-due-date-cashflow`). ✅ `feature-due-date-cashflow.md` (Feature: Expected Payment Date, see Phase History) executed end-to-end and deployed 2026-09-13 — deleted from `prompts/`. ✅ 2026-07-04 run-all-phases session: 7 phases (Repair 14, Approval Notification, Attendance Admin, Billing Events Engine, Billing Reports Accuracy, Form Improvements, Mobile Redesign) executed end-to-end and deployed; all 8 completed `.md` prompt files for this session deleted from `prompts/` (history lives in git + the Phase History rows below); only the pre-existing `run-all.ps1` runner script remains. ✅ Repair 6 fully complete (commit 71df2f4): every `supabase.from(...)` write in the app now has a 20s abort — coverage-grep verified zero gaps. Only `auth.*`/`functions.invoke`/`storage.*` excluded (different APIs). See Repair 6 Phase History note. ✅ 2026-07-13 `fix-save-rootcause-auth-deadlock.md` completed and deleted from `prompts/` — see "Fix: Save-Timeout Root Cause" Phase History row below; **this row supersedes Repair 6 and Repair 14's write-layer diagnosis** — those fixes were real (abortSignal coverage, functions.invoke timeouts) but not the cause of the recurring bug users kept hitting. ✅ `batch8-mobile-reports-giyus-billing.md` (5 phases: mobile hard lock, השמה→גיוס merge, advance billing engine fix, דוחות section, מנהלה billing permission + הדרכה auto-generation) executed end-to-end and deployed across two sessions — phases 1-4 on 2026-08-23, phase 5 on 2026-09-06 (the file's own step-4 cleanup note said "once all four phases are live", a stale count left over from an earlier draft of the prompt; its header always said five) — deleted from `prompts/` only after phase 5 landed; also deleted the never-executed, fully-superseded `repair15-advance-billing-event.md` as part of that batch's Phase 3 commit — see Phase History rows below.
...
billing_events                     -- Source of truth for all money events (added Phase 2)
  ...
  payment_date date                -- ACTUAL receipt date only (when money arrived) — see Feature: Expected Payment Date below. Auto-set to today when receipt_number is entered and it was empty; editable via "תאריך תשלום בפועל" in TransactionDialog.
  due_date date                    -- EXPECTED payment date (תאריך תשלום צפוי / תאריך פירעון) — trigger-maintained, see below. Added 20260913_due_date.sql.
...
- `due_date` — expected payment date = `end_of_month(billing_date) + payment_term_days`, **DB-trigger-maintained** (see Feature: Expected Payment Date below), can be manually overridden (`due_date_is_manual`)
- `payment_date` — the ACTUAL date money arrived. Auto-set to today when `receipt_number` is entered and it was empty; independently editable via "תאריך תשלום בפועל"
...
- ⚠️ Before 2026-09-13, `payment_date` was overloaded to mean both of the above — see Feature: Expected Payment Date in Phase History for why they were split.
...
`TransactionDialog.handleSave` does NOT write `transactions`/`billing_events` directly.
It computes the billing-events drafts client-side (unchanged — `generateServiceBillingEvents`/
`reconcileFinalSalaryBillingEvents` above) and sends them, alongside the transaction payload,
to a single RPC: `supabase.rpc('save_transaction_with_events', { p_mode, p_id, p_payload,
p_events, p_flip_to_bill, p_work_end_date })` (migration `20260712_atomic_save_transaction.sql`).
...
can never leave an orphan transaction row without its billing events. Any NEW column added to
either table's save payload must be added to the RPC's explicit column list (both the `update`
`case when p_payload ? '...'` branch and the `insert` values list) — it will otherwise be
silently ignored, not written.
...
- **`billing_events.due_date` = expected payment date (trigger-maintained), `payment_date` = actual receipt date — never conflate them again** (Feature: Expected Payment Date, 2026-09-13). Bonus month attribution (`bonus.ts`) keys off `due_date`.
...
| Fix: Save-Timeout Root Cause | 20260712_atomic_save_transaction.sql | ✅ Live (2026-07-13) | Ended the recurring "השמירה לא הושלמה — פג זמן" bug that Repairs 6 and 14 had each partially misdiagnosed as a write-layer/abort-plumbing gap (both fixes were real but didn't touch the actual cause). **Proven root cause** (verify-first: reproduced with runtime evidence before changing any code): `src/lib/auth.tsx`'s `onAuthStateChange` callback awaited `supabase.from('profiles')...` directly inside the callback body. supabase-js (`@supabase/auth-js` 2.104.0) serializes auth operations behind an internal per-tab lock (`GoTrueClient#_acquireLock`); the callback runs *while that lock is held*, and a nested `getSession()` call (which every `supabase.from()` request makes internally to attach the access token) hits a reentrancy fast-path that chains onto the very outer operation awaiting the callback — a genuine circular wait with **no timeout and no recovery**, unlike the bounded 5s `navigator.locks` path. Confirmed live via a local dev server against production Supabase (isolated, no prod data touched): forcing `refreshSession()` then immediately saving a transaction reproduced the exact Hebrew timeout message, with the console trace showing `before profiles.select` logged and **never** followed by `after profiles.select`, and the network panel showing the `transactions` write **never dispatched at all** — while the tab remained otherwise fully responsive (ruling out the unrelated CDP/tab-freeze artifact documented in the Mobile Redesign row above, which is a different, already-understood automation-harness issue, not this bug). **Fix 1** — `auth.tsx`: `onAuthStateChange`'s callback now does *only* synchronous `setState` calls; every Supabase data call (the profile fetch, `syncProfileEmailIfStale`) is deferred via `setTimeout(fn, 0)` so it runs after the callback returns and the lock is released. Recovery flow (`recoveryMode`, `bhr_recovery_mode`, `/set-password` gating) behavior is unchanged. **Fix 2** — new `public.save_transaction_with_events(p_mode, p_id, p_payload, p_events, p_flip_to_bill, p_work_end_date)` RPC (`SECURITY DEFINER`, re-implements the `transactions_full_access` USING+WITH CHECK rule since it bypasses RLS): `TransactionDialog.handleSave` now computes the billing-events drafts in TypeScript exactly as before (`generateServiceBillingEvents`/`reconcileFinalSalaryBillingEvents` in `lib/billingEvents.ts`, untouched) but sends them alongside the transaction payload to this ONE RPC call instead of 4–5 sequential round-trips (transaction write → billing_events upsert → pending→to_bill flip → cancel-future-events), so a stall or error partway through can no longer leave an orphan transaction row without its billing events. The approval email (`send-approval-email`) stays a separate best-effort post-save call. QA: build/tsc clean; migration applied via direct `pg` connection (Management API token still expired, same as noted elsewhere) and confirmed via `pg_proc`; atomicity verified by forcing a malformed billing-event cast inside the RPC and confirming zero orphan transaction rows persisted; full live save-flow verified for both a גיוס and a הדרכה transaction (create → DB-confirmed persistence incl. correct billing_events split math → reopen → every field reloaded correctly) via a `[TEST-Deadlock]`-tagged throwaway admin account + client link, all cleaned up and confirmed via a final DB sweep (zero rows remain); regression 12/12 Transactions columns confirmed both on the local repro environment and read-only on live production. Deploy confirmed via bundle-hash match (`index-Aw_wyDzc.js` identical between local `dist/` and `https://app.banani-hr.com`) since the `VERCEL_TOKEN` in `.env.local` is also expired (403 `invalidToken`, same pattern as the Supabase Management API token — both need renewing). ⚠ Did not sign out or use Oren's own already-authenticated real session on `app.banani-hr.com` for any write testing — only read-only checks (columns, console, RTL) were run there; all save/atomicity repro used a throwaway test admin against a local dev server pointed at the same production Supabase project. Commit b76167b. |
...
| Batch 8 Phase 1: Mobile Hard Lock | (no migration) | ✅ Live (2026-08-23) | Root cause: `MobileAutoRoute`'s one-shot `useRef` guard skipped re-checking after the first redirect (Defect A), and the `bhr_force_desktop` localStorage opt-out permanently reproduced the desktop sidebar on a phone once set (Defect B) — both confirmed exactly as hypothesized before coding. New `src/hooks/useIsMobileDevice.ts` (UA regex OR narrow-width+coarse-pointer, reactive) is the single source of truth. `MobileAutoRoute.tsx` rewritten: no `ran` ref, no opt-out read (actively clears the stale key on mount). New render-boundary guard in `RequireRole.tsx` closes the one-frame flash the effect-only approach couldn't (`/m` routes pass `withLayout={false}` so can never loop into it). Removed the now-dead "תצוגת דסקטופ" button from `MobileShell.tsx` — an unplanned "reverse counterpart" the audit was asked to check for and found. Live QA via UA-override + resize-event dispatch (the `resize_window` tool workaround documented in earlier phases): both defects confirmed fixed live, desktop unaffected. Commit `ef5840c`. |
| Batch 8 Phase 2: השמה → גיוס Merge | (no migration — data only) | ✅ Live (2026-08-23) | ⚠ Found the prompt's Step 1 assumption wrong: `candidate_number`/`final_salary` (added in the Form Improvements phase) are hardcoded `TransactionDialog.tsx` renders keyed off `isGiyus`, NOT `service_types.fields` jsonb entries — גיוס's actual fields row only has `position_name, candidate_name, salary, sign_date, work_start_date`. Implemented `position_number` the same hardcoded way (rendered directly above `position_name`) instead of adding it to the DB jsonb array — zero schema change, exact field order Oren asked for. Also found השמה's candidate-number key (`candidate_no`) doesn't match גיוס's (`candidate_number`) as the prompt assumed — no data impact, never populated on any of the 24 migrated rows. Migrated all 24 השמה transactions to גיוס in one idempotent `BEGIN...COMMIT` via the pooler (service_type/service_type_id flip, `work_end_date` COALESCEd from the unused `custom_fields.termination_date`, `custom_fields.final_salary` populated from the existing salary — source keys left in place, non-destructive). Billing events (42) verified unchanged. Backed up all 24 rows before writing (`backups/`, gitignored). השמה service type intentionally left in `/services` for Oren to delete manually. DB-verified + live-verified (migrated record field order/values correct; `[TEST-Batch8P2]` round-trip on a throwaway local-dev admin confirmed `position_number` persists, cleaned up). Commit `c23838c`. |
| Batch 8 Phase 3: Advance Billing Engine | (no migration) | ✅ Live (2026-08-23) | Root cause confirmed: `handleSave`'s old `shouldGenerateEvents` gate required `work_start_date`, so a freshly-opened גיוס (no candidate/start-date yet) generated zero billing events, advance included — not the field-name mismatch the never-executed `repair15-advance-billing-event.md` guessed (deleted this commit). Money-rule fix confirmed with Oren 2026-08-22: advance = % **of the commission** (`salary × commissionPct/100`), not of gross salary — `resolveAdvanceAmount()` in `billingEvents.ts` rewritten accordingly, capped at total commission for both fixed/percent types; `AdvanceEditor.tsx` labels updated (אחוז מהשכר → אחוז מהעמלה). `generateServiceBillingEvents()` now accepts `workStartDate: string | null` + `advanceBillingDate` — emits the advance alone (from `entry_date`) when there's no work-start date, skips split generation. `handleSave` splits `shouldGenerateEvents` into `canGenerateSplits`/`canGenerateAdvanceOnly`; a transaction holding only an advance-only event takes the full regeneration path (not reconcile) once a work-start date is added, so the RPC's delete-then-insert never produces a duplicate index-1 row. Legacy `GenerateBillingEventsButton` had the identical missing-advance bug (hardcoded `advanceAmount: 0`), fixed the same way. Proven with a 9-scenario/32-assertion standalone math script (`node --experimental-strip-types`) before any live test, then a full live E2E on a `[TEST-Batch8P3]` throwaway client+transaction (advance-only creation, work-start-date-triggered full regeneration with no duplicate index-1, final-salary reconciliation) — all DB-exact, cleaned up. Zero production clients affected by the formula change today (both percent-advance clients are at 100% commission, where old/new formulas coincide). Commit `8487d66`. |
| Batch 8 Phase 4: דוחות Section | (no migration) | ✅ Live (2026-08-23) | New admin-only `/reports` route + sidebar item (after דוחות חיוב): `src/pages/Reports.tsx` — report picker (one report today, one-line to add another), date-range (default 1 Jan → today) + מוביל/לקוח/search filters, 5-card KPI row, three sortable breakdown tables (לפי מוביל/לקוח/משרה) with single-row drill-down, all aggregated client-side from two queries (גיוס transactions in range + their paid billing_events) — no new DB objects. New `src/lib/excelExport.ts` (no prior export helper existed) for the 4-sheet Excel export. ⚠ **Build broke and was fixed same phase**: the new page's bundle weight crossed vite-plugin-pwa's 2 MiB default precache limit, failing the production build; a dynamic `import('xlsx')` didn't help (Clients.tsx already statically imports `xlsx`, so the bundler can't chunk-split it regardless — confirmed by the `INEFFECTIVE_DYNAMIC_IMPORT` warning) and was reverted; fixed instead by raising `workbox.maximumFileSizeToCacheInBytes` to 3 MiB in `vite.config.ts`, the fix vite-plugin-pwa's own error message points to. Every KPI and per-lead breakdown number cross-checked against a direct SQL aggregate through the pooler — exact matches across the board. Live QA: filters narrow all blocks correctly, drill-down correct, empty state confirmed, Excel export downloaded + read back with `xlsx` to verify 4 sheets/Hebrew/exact numbers. Role guard confirmed both by code (`RequireRole allow={['admin']}`, matching the established `/bonuses`-style pattern) and live — a real non-admin session on the shared browser was correctly redirected away from `/reports` with no sidebar item, read-only, no further interaction with that real account. Commit `3e7f19e`. |
| Batch 8 Phase 5: מנהלה Billing Permission + הדרכה Auto-Generation | (no migration) | ✅ Live (2026-09-06) | Two fixes for a reported dead-end: opening a הדרכה transaction as `administration` showed "אין אירועי חיוב לעסקה זו." with **no** "+ הוסף אירוע" button, unlike admin. **Root cause 1 (UI-only gate)**: `TransactionDialog.tsx`'s `isAdmin` (single use site, gating the button) → replaced with `canManageBillingEvents = admin \|\| administration`. **Runtime-proven, not assumed**: created a throwaway `administration` profile, signed in for a real JWT, and confirmed a genuine PostgREST `INSERT` into `billing_events` already succeeds — `administration_billing_events_all`'s `FOR ALL USING(...)` with no `WITH CHECK` means Postgres reuses `USING` (role-only, no row reference) for `WITH CHECK` too, so no migration was ever needed. **Root cause 2 (no auto-generation)**: הדרכה never has a `work_start_date`, so `GenerateBillingEventsButton`'s render gate never fires — manual add was the *only* path, and root cause 1 closed even that for administration. Fixed with a new `generateHadrachaBillingEvent()` in `billingEvents.ts` (single event, amount = the live total the dialog already computes so it can never drift from what's displayed, `billing_date` = the latest valid execution date validated against `^\d{4}-\d{2}-\d{2}$` — a real malformed date string was found in live data — falling back to `entry_date`, returns `null` for amount ≤ 0) wired into `handleSave` as a third `canGenerateHadracha` track alongside Phase 3's `canGenerateSplits`/`canGenerateAdvanceOnly`. ⚠ **Bug caught by testing the RPC directly before any UI test**: `save_transaction_with_events`'s "delete pending/to_bill, reinsert only what's in p_events" logic deletes by status only, not by index — since the הדרכה generator only ever emits index 1, a manually-added index-2 event (still `pending`) was being silently wiped on every regeneration. Fixed by having the הדרכה branch also carry forward any existing non-index-1 pending/to_bill events into `p_events` so the RPC reinserts them unchanged. The locked-event case needed no RPC fix (occupied-index skip already protects it) but gained a Hebrew warning (`אירוע החיוב של הדרכה זו כבר חויב...`) reusing the existing `window.alert(reconcileWarning)` call site. QA: build/tsc clean; regression 12/12 (Transactions.tsx untouched); full live E2E on throwaway `[TEST-Batch8P5]` admin + administration accounts + client (all cleaned up, final DB sweep = 0 rows) — single-event creation, 2nd-execution-date + travel amount math (drift-tested against the on-screen total), locked-event-unchanged-plus-warning, manual-event-survives-full-regeneration, zero-price-no-event, and the administration-role add→invoice→receipt flow end-to-end all confirmed exactly; גיוס re-verified unaffected (advance-only ₪2,250 on create, then exactly one locked advance + one new split totalling ₪7,500 after adding a work-start date — no duplicate index-1 row). Deploy confirmed via bundle-hash match (`index-CfUqpPSq.js`). Commit `70db13e`. |
| Feature: Expected Payment Date | 20260913_due_date.sql, 20260913_2_due_date_freeze_paid.sql | ✅ Live (2026-09-13) | AdminDashboard's "לוח תשלומים צפוי" widget was showing `—` for almost every row: the expected payment date was only ever computed on-the-fly in `TransactionDialog` and written to `billing_events.payment_date` as a side effect (when a receipt number was entered), so `payment_date` was overloaded to mean both "expected" and "actually received" and the widget (reading straight from the DB) saw mostly NULLs. **Fix**: new `billing_events.due_date` + `due_date_is_manual` columns, computed by a `BEFORE INSERT OR UPDATE` trigger (`trg_billing_events_due_date` → `bhr_billing_events_set_due_date()`), using DB-native ports of the existing TS formulas (`bhr_calc_due_date`/`bhr_parse_payment_term_days`, kept in lockstep with `calculateTaxInvoiceDate`/`parsePaymentTermDays` in `lib/billingEvents.ts` — the TS versions remain only as the live in-dialog preview). A second trigger (`trg_clients_recompute_due_dates`) recomputes a client's still-open events' `due_date` when `payment_terms` changes, deliberately excluding `paid`/`cancelled` so a settled bonus month can't be silently moved (D5). Retroactive backfill applied to all existing rows (manual `payment_date` overrides on non-paid events preserved as `due_date_is_manual=true`). `payment_date` now means ONLY the actual receipt date — auto-set to today when a receipt number is entered and empty, independently editable via a new "תאריך תשלום בפועל" field in `TransactionDialog`; the existing "תאריך פירעון" field now reads/writes `due_date` with a DB-driven (מחושב)/(ידני) badge + `↩` reset. `AdminDashboard`'s widget now reads `due_date` and groups rows into per-month cash-flow subtotals (nulls sort last under "ללא תאריך"); `BillingReports` gained a second sortable "תאריך פירעון" column alongside the existing "תאריך תשלום". **Bonus month attribution (D5)** in `bonus.ts` now keys off `due_date` (falls back to `payment_date`, then `billing_date`) instead of `payment_date` — the paid-only gate from Repair 4 is unchanged, only which month a paid event counts toward; this does shift several employees' historical monthly bonus numbers (verified via a before/after DB query — e.g. Noa's May net roughly doubled and June roughly halved once re-keyed by due date — call this out to Oren if bonus payouts are questioned). ⚠ **Bug caught live during QA, fixed same day** (`20260913_2_due_date_freeze_paid.sql`): the billing_events trigger recomputed `due_date` on **any** update to the row, not just ones touching `billing_date`/terms — so saving an unrelated field (e.g. the new "תאריך תשלום בפועל", or correcting a receipt number) on an already-`paid` event silently moved its `due_date` to match the client's *current* terms, defeating the whole point of freezing paid events. Reproduced with a no-op `amount = amount` update on a paid test row. Fixed by having the trigger skip auto-recompute whenever `OLD.status IN ('paid','cancelled') AND NEW.status = OLD.status` (a manual override still always wins, checked first). QA: build/tsc clean; migration applied via pooler, `§3.1(h)` verification query showed 0 rows with a `billing_date` but NULL `due_date`; a rolled-back SQL transaction proved the client-terms trigger correctly leaves paid events untouched while shifting open ones; live E2E on a throwaway `[TEST-DueDate]` admin+client+transaction (created via Admin API, since no login credentials were available) confirmed the (מחושב)→(ידני) badge flip on manual edit, the `↩` reset-to-computed flow, receipt-number→paid+payment_date=today without touching due_date, and — critically — reproduced and then re-verified the fix for the paid-row-freeze bug against the live DB; `AdminDashboard` widget and `BillingReports` sortable column confirmed visually on real production data (had to force-clear the PWA service worker cache mid-QA — same gotcha as earlier phases — to see the new bundle). All test data (auth user, profile, client, transaction, billing event) deleted and confirmed via a final DB sweep = 0 rows. Deploy confirmed via byte-identical bundle-hash match. Commits `222af7a` (feature), `356959a` (paid-freeze fix). |
