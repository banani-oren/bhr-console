...
## 35. UX bug fixes (Batch 4 Phase A — 2026-04-22)

- [x] **Client name shows correctly.** `ClientPicker` component renders
      the selected client's `name` in its trigger in every consumer
      (`/hours` page selector, hours-entry dialog, `TransactionDialog`,
      `/billing-reports` filter, `/m/hours` sheet). A stale/unloaded
      value never bleeds through as a UUID fragment.
- [x] **Save hang.** `useSafeMutation` wraps the two hang-prone saves
      (`/clients` save + hours-log insert) with a 15 s abort timeout +
      a consistent `idle/saving/success/error/timeout` state machine.
      Timeouts surface `פג זמן השמירה. נסה שנית.` and re-enable the
      dialog.
- [x] **Dialog widths + close button.** Shared `DialogContent` places
      the close ✕ at `top-2 end-2 z-50` (RTL-aware). `DialogHeader`
      reserves `pe-10` so long titles wrap instead of colliding with
      the close button. Existing consumers keep their per-dialog
      `max-w-*` classes; the conventional scale is sm / lg / 4xl.
- [x] **Toggle clarity.** `LabeledToggle` wraps `Switch` with adjacent
      off/on text (bold active side, purple-600 track when on).
      Migrated: `is_billable`, `exclusivity`, `time_log_enabled`,
      `hours_category_enabled`, `bonus_enabled`, include-service /
      include-time-period on `/billing-reports`.

## 36. Hours-log workflow in-dialog (Batch 4 Phase B — 2026-04-22)

- [x] **Client picker in dialog.** `+ הוסף דיווח` on `/hours` is
      always clickable when the user has at least one permitted client;
      the dialog's first field is a `ClientPicker` pre-filtered to the
      current user's permitted time_log_enabled clients.
- [x] **Pre-seed + edge case.** If the top-of-page client filter is set,
      the dialog pre-selects that client (still editable). If the user
      is permitted on zero clients, the dialog renders
      `אין לקוחות מורשים לדיווח שעות. פנה למנהל.` and save is disabled.

## 37. Flexible billing reports (Batch 4 Phase C — 2026-04-22)

- [x] **Schema.** `billing_reports` gains `filter_client_id`,
      `filter_period_start`, `filter_period_end`, `filter_payment_status`,
      `filter_include_service` (default true), `filter_include_time_period`
      (default true). `client_id`/`period_start`/`period_end` are now
      nullable so multi-client / all-time reports can be stored.
- [x] **Any combination.** Filter strip accepts client-only, period-only,
      both, or neither; payment_status filter; include-service /
      include-time-period toggles (both on by default).
- [x] **Multi-client PDF.** `buildBillingReportPdf` groups rows by
      client with subtotal headers when the filter is "all clients";
      single-client reports keep the existing per-row table + expanded
      hours pages for each included `kind='time_period'` transaction.
- [x] **Broad-scope warning.** No client + no period + >200 rows
      selected → confirmation modal
      (`אתה עומד להפיק דוח עם יותר מ-200 שורות. להמשיך?`).
- [x] **De-dup.** Grey-out + disabled checkbox for transactions
      already included in an earlier report; applies only in the
...
- [x] **Manifest + icons + service worker.** `vite-plugin-pwa` wired;
      manifest declares `lang=he / dir=rtl / display=standalone`;
      192/512/512-maskable icons + 180 apple-touch-icon generated by
      `scripts/generate-icons.mjs`. Build emits `dist/sw.js` +
      `dist/workbox-*.js` and precaches 14 entries (~2.4 MB). Supabase
      API uses `NetworkFirst` w/ 24 h TTL; `/auth/*` is `NetworkOnly`.
- [x] **Install UX.** Admin sidebar footer shows `התקן BHR Console`
      button when `beforeinstallprompt` fires + not standalone;
      `/login` renders a Share → Add-to-Home-Screen hint when UA is
      iOS Safari + not standalone.
- [x] **`/m` route group.** `MobileShell` with bottom-tab nav
      (שעות / משרות / פרופיל); `/m/hours` with bottom-sheet entry
      form + 14-day grouped list; `/m/transactions` read-only cards;
      `/m/profile` with device status + install guidance + sign-out.
- [x] **Auto-route.** `MobileAutoRoute` redirects non-admin narrow
      viewports (<640 px) to `/m/hours` on first authenticated load.
      Admins default to the desktop shell.
- [x] **Offline queue.** `src/lib/offlineQueue.ts` (idb-keyval)
      persists hours_log inserts while offline; banner at top of
      `/m/hours` shows pending count with a retry button that flushes
      the queue when the device is back online.
- [x] **Biometric-friendly login.** `/login` form uses `method="post"`
      with `autoComplete="username"` on the email input (plus
      `inputMode/spellCheck/name`) and `autoComplete="current-password"`
      on the password input; `/set-password` includes a hidden
      `autoComplete="username"` email mirror. Supabase client is
      explicit about `persistSession/autoRefreshToken/detectSessionInUrl`.
      Supabase Auth config patched to `jwt_exp=3600 +
      refresh_token_rotation_enabled=true` so active sessions refresh
      silently. Face-ID behavior itself must be validated on a real
      iPhone (desktop Chrome can't exercise Keychain autofill).
