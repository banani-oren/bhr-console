...
// ---------------------------------------------------------------------------
// Phase 3: Performa (PROFORMA) invoice PDF for billing events
// ---------------------------------------------------------------------------

export function generatePerformaPDF(
  events: Array<{ description: string | null; billing_date: string | null; amount: number }>,
  clientName: string,
  fileName?: string,
): void {
  ...
  doc.text('BHR - Banani HR', 105, 20, { align: 'center' })

  doc.setFontSize(10)
  doc.setFont('helvetica', 'normal')
  doc.text('bananioren@gmail.com', 105, 27, { align: 'center' })
  ...
  doc.setFontSize(14)
  doc.setFont('helvetica', 'bold')
  doc.text('PROFORMA INVOICE / חשבון עסקה', 105, 40, { align: 'center' })

  // Client + date
  doc.setFontSize(10)
  doc.setFont('helvetica', 'normal')
  doc.text(`Client / לקוח: ${clientName}`, 196, 52, { align: 'right' })
  doc.text(`Date / תאריך: ${todayStr}`, 196, 59, { align: 'right' })

  const rows = events.map((ev) => {
    const dateStr = ev.billing_date
      ? new Intl.DateTimeFormat('he-IL', { day: '2-digit', month: '2-digit', year: 'numeric', timeZone: 'Asia/Jerusalem' }).format(new Date(ev.billing_date))
      : '—'
    return [ev.description ?? '—', dateStr, ils(ev.amount)]
  })

  const total = events.reduce((s, ev) => s + ev.amount, 0)

  autoTable(doc, {
    startY: 68,
    head: [['Description', 'Billing Date', 'Amount (ILS)']],
    body: rows,
    foot: [['', 'Total', ils(total)]],
    styles: { halign: 'center', fontSize: 9 },
    headStyles: { fillColor: [124, 58, 237], textColor: 255, fontStyle: 'bold' },
    footStyles: { fillColor: [237, 233, 254], textColor: [55, 13, 90], fontStyle: 'bold' },
    columnStyles: { 0: { halign: 'right' }, 1: { halign: 'center' }, 2: { halign: 'left' } },
  ...
  doc.text('Please arrange payment by the billing date indicated above.', 105, finalY + 12, { align: 'center' })
  doc.text('For bank transfer details, please contact us.', 105, finalY + 18, { align: 'center' })

  doc.setFontSize(8)
  doc.setTextColor(150)
  doc.text(`Generated by BHR Console · ${todayStr}`, 105, 285, { align: 'center' })

  doc.save(fileName ?? `performa-${clientName}-${todayStr.replace(/\//g, '-')}.pdf`)
}
